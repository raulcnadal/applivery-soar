...
`docker-publish.yml` publishes two multi-arch images to GitHub Container Registry (ghcr.io), attached to this repo, on every push to `main`: `ghcr.io/raulcnadal/applivery-soar` (backend/Dockerfile) and `ghcr.io/raulcnadal/applivery-soar-frontend` (frontend/Dockerfile), using the workflow's own `GITHUB_TOKEN` — no external registry credentials to manage. These packages are private, so a deployment host needs a one-time `docker login ghcr.io` (a GitHub PAT with `read:packages` scope) before its first pull. Two supported deployment shapes both pull from these, no local build required:
