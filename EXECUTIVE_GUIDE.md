...
The module is delivered as a hosted Applivery service — a Vue 3 dashboard and a Node.js/Express backend, operated and maintained by Applivery as part of the platform, with nothing for a customer to install, patch, or run themselves — and with no separate user database: every admin authenticates with their real Applivery credentials, and access is governed by roles SOAR manages on top of that identity.
...
A **Compliance Policy** is a machine-evaluated rule set that automatically links to a remediation Workflow, built from a condition catalog spanning 54 distinct, purpose-built fields — OS version and patch level (compared platform-natively: Android security-patch date, Apple version+build, Windows full build number), disk encryption, firewall state, jailbreak/root detection, screen lock, Secure Boot/TPM/Applivery compliance flags, check-in recency, risk score/tier, required or disallowed apps against an App List, geofence zone membership and location freshness, MITRE ATT&CK-tagged detections, and any admin-defined Custom Device Check result — each with its own applicable comparison operators (equals, greater/less-than, contains, and more) and, where relevant, a curated options list, combinable under **Match ANY** or **Match ALL** logic. This breadth is deliberate: it lets an admin express almost any real-world posture requirement as a policy condition without waiting on a new platform release for it.

Policies run on a schedule the organization controls, but don't only wait on it: enabling a policy, changing which devices it targets, or a device's own agent reporting a new attribute value each independently trigger an immediate re-check, so a real change in scope or posture doesn't sit unevaluated for up to the schedule's own interval. Every violation is tracked, whether it auto-remediates immediately or is queued for human review first, and a violated policy is fully explainable: clicking it from a device's own record opens a per-condition breakdown showing exactly which conditions are met and which aren't, with a clear pass/fail indicator per condition and the last evaluation time — no need to reverse-engineer which of a policy's many conditions actually tripped. Alerting is per-channel: Webhook and Email notifications can each independently carry an optional daily send cap, so a misconfigured or overly broad policy can't flood an inbox or a downstream system, while a channel that should stay unlimited can be left uncapped.

A **Template Gallery** ships 61 ready-made policies mapped directly to **ISO 27001**, **ENS** (the Spanish National Security Framework), and **NIS2** Article 21 controls — each written for exactly one operating system's real, checkable security signal (or explicitly marked platform-agnostic where a control genuinely applies the same way everywhere) rather than one generic rule stretched thin across every OS, so a template an admin adopts is immediately checkable against something that platform actually reports. An **App Lists** view (under Apps) enforces mandatory/disallowed software catalogs per platform, feeding the same policy engine.

**Value**: compliance stops being a point-in-time snapshot taken before an audit and becomes a continuously enforced, fully explainable state, with named framework alignment auditors can actually follow, and safety limits (auto-run batch caps, destructive-action acknowledgment, per-channel alert caps, an automatic circuit breaker after repeated failures) that keep automation precise and predictable at any fleet size.
...
### 3.9 On-device telemetry & the native agents — data Applivery's own API doesn't expose

A lightweight native background service (a Windows Service / macOS LaunchDaemon, distributed as a signed installer) runs on managed desktop endpoints and reports the security signals Applivery's own MDM API doesn't surface — disk encryption, firewall state, Secure Boot/TPM/FileVault/XProtect status, and installed-application inventory — directly into the compliance engine on a schedule. Beyond that fixed set, **Custom Device Checks** let an admin define an arbitrary additional probe with no code and no new agent release — is a named process or service running, does a registry value or config file hold an expected value, is a specific application installed at a required version, or (for teams that need it) run an arbitrary command and evaluate its output — and have the result available as a Compliance Policy condition on the very next device check-in.

A companion **SOAR Mobile Agent** (iOS and Android) closes the same visibility gap on mobile, where an MDM API structurally can't expose as much: device passcode/encryption status, Android's SSL/TLS security-provider currency and hardware-backed key-storage attestation, a live Google Play Integrity verdict confirming the device and app haven't been tampered with, and an in-house runtime self-protection layer detecting root/jailbreak, an attached debugger, or a Frida/Xposed-style hooking framework — the same class of anti-tampering evidence a dedicated RASP product would provide, built directly into the platform rather than requiring a separate vendor and its own data-residency negotiation. Every one of these signals is a friendly, discoverable Compliance Policy condition, not a raw technical field an admin has to already know the name of, and both the Windows/macOS and mobile agents let an end user (or the admin at that device) trigger an immediate compliance re-check or telemetry push on demand, rather than waiting for the next scheduled cycle.

The agents themselves are distributed with the same zero-friction model as the platform's own container images: no separate download portal, no manual credential exchange — a fresh build is available directly from Settings the moment it's published, and the desktop agents can be pushed into Applivery with one click, from an admin's own already-authenticated session, leveraging native Applivery UEM app deployment to get them onto devices fleet-wide.

**Value**: closes the gap between "what Applivery's MDM API reports" and "what a security team actually needs to verify" — across every managed platform, not just desktop — without waiting on a platform release for every new check an auditor or a threat asks for, and without paying for a separate mobile threat-defense or RASP product to get tamper-evidence on phones and tablets.

### 3.10 Settings — administration, integrations, and intelligence feeds

The control plane for the entire platform: role-based access control, every third-party integration (below), and a genuinely broad set of vulnerability and OS-update intelligence catalogs that run themselves on a schedule with zero required configuration and feed directly into Devices, Apps, and Compliance:

- **OS update & lifecycle tracking** — Microsoft's own Security Update Guide for Windows patch status, Apple's GDMF build-signing feed for the latest signed iOS/iPadOS/macOS builds, and endoflife.date for OS end-of-life dates across every platform.
- **CVE / vulnerability catalogs** — ENISA's EU Vulnerability Database, MISP (Malware Information Sharing Platform) threat-event feeds, and VulnCheck's vulnerability intelligence, each refreshed on its own background schedule.
- **Platform-specific CVE matching** — OSV.dev's Android Security Bulletin ecosystem for Android OS-level CVEs, and Apple's own SOFA (Simple Organized Feed for Apple) security-release feed for macOS/iOS/iPadOS CVEs — both matched against a device's actual patch level, not just its major OS version.
- **Binary Integrity** — a VirusTotal file-reputation lookup against the SHA256 hash of installed executables the native agent reports, flagging sideloaded or tampered binaries independent of CVE-based matching.
- **Vulnerability Service (optional, richer)** — Applivery's own hosted per-app CVE matching across all four platforms including Windows, with CISA Known Exploited Vulnerabilities (KEV) flagging and a 0–100 risk score per app, rolling up into the same per-device Risk Score used throughout Devices and Overview.

**Value**: an admin gets fleet-wide vulnerability and patch-currency visibility on day one, sourced from multiple independent, continuously-refreshed feeds, without standing up or paying for a separate vulnerability-management product.
...
| Threat intelligence | VirusTotal | Outbound (on-demand) | Analyst-triggered IOC enrichment (IP/domain/URL/hash) from inside a Case, with a 6-hour result cache. |
...
| On-device telemetry | The native Applivery SOAR Agent (Windows Service / macOS LaunchDaemon), the SOAR Mobile Agent (iOS/Android), or a lighter-weight optional self-report script (macOS `.sh` / Windows `.ps1`) for teams that prefer scheduling it themselves | Inbound | Reports attributes Applivery itself doesn't expose — disk encryption, screen lock, antivirus status, Secure Boot/BitLocker/TPM/FileVault/XProtect state, installed-app inventory, any admin-defined Custom Device Check (desktop only), and — on mobile — passcode/encryption status, Play Integrity verdicts, and in-house RASP root/jailbreak/debugger/hooking-framework detection — back into the compliance engine via a dedicated webhook. |
| Vulnerability & OS-lifecycle intelligence | ENISA EU Vulnerability Database, Microsoft MSRC, endoflife.date, Apple GDMF, MISP, VulnCheck, OSV.dev (Android Security Bulletin), SOFA (Apple security releases) | Inbound (automatic, no configuration) | A multi-source set of no-signup data feeds refreshed on independent background schedules, powering the CVE, patch-gap, patch-level-aware Android/Apple CVE matching, and OS-lifecycle signals used throughout Devices, Apps, and Compliance. |
| Binary integrity | VirusTotal (file-reputation) | Outbound (automatic) | Hashes installed executables the native agent reports and flags sideloaded/tampered binaries, independent of CVE-based vulnerability matching. |
| Optional richer vulnerability intelligence | Applivery's own hosted Vulnerability Service | Bidirectional (opt-in) | A deeper, cross-platform (including Windows) per-app CVE-matching service with confirmed fix versions and CISA KEV exploit-likelihood flagging, feeding a 0–100 risk score per app and per device, for organizations that want more than the free public feeds provide. |
...
- Roles are granular in two independent dimensions: a three-way (No access / View only / Manage) toggle across eight functional areas (Devices, Compliance Policies, Workflows, Cases, Integrations & Threat Intel, Reporting, Audit Log, and Settings), each independently enforced on every backend route, plus five independent high-risk permission flags that gate specific dangerous actions regardless of area-level access: deleting a policy or workflow, running a workflow containing a destructive device action, editing integration/threat-intel secrets, exporting or importing workspace configuration, and bulk-triaging violations or cases.
...
Every credential a workspace admin configures in Settings — Jira/ServiceNow ticketing credentials, PagerDuty/Opsgenie keys, chat webhook URLs, Threat Intel provider API keys, the SMTP password, Google Play Integrity's decryption/verification keys, and the optional Vulnerability Service API token — is encrypted at rest using industry-standard symmetric encryption (Fernet/AES), keyed from the same secret that signs dashboard sessions. The Play Integrity setup goes a step further: the RSA private key an admin generates locally to decrypt Google's response is used once, server-side, at save time and never stored at all — only the two resulting keys it decrypts to are persisted, encrypted. Storage is the only layer this protects by design: an admin with a valid, permitted session still sees the real value where the product needs to show it (e.g. an integration's edit form), matching how every other settings screen in the platform already works; what's no longer true is a plaintext credential sitting readable in the on-disk database file itself. The Vulnerability Service token goes one step further and is never echoed back to the browser at all after being saved, showing only a masked last-four-characters hint on later edits — the strictest handling in the platform, reserved for a credential with no legitimate reason to ever be re-displayed.

The Applivery inbound webhook's own shared secret uses a complementary model rather than encryption-at-rest: it's a self-generated random token embedded directly in a URL path rather than an admin-entered credential, so its security posture is rotation-on-suspicion (§5.5) — any workspace can rotate it instantly and independently the moment it's suspected of leaking.
...
Applivery hosts and operates SOAR as part of the platform, behind the same production infrastructure and network controls that already protect Applivery UEM/MDM — a customer has no server to patch, no reverse proxy to configure, and no separate infrastructure to secure. Every workspace's data lives in its own isolated store within that hosted environment (§5.3), and the service fails closed rather than fails open on misconfiguration: it will not start at all without its session-signing secret and database connection explicitly set, rather than falling back to an insecure default.
