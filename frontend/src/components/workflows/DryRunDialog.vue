  ...
  <Modal :open="open" :title="workflow ? `Dry run — ${workflow.name}` : 'Dry run'" size="lg" @close="emit('close')">
    <div class="space-y-4">
      ...
      <template v-else-if="result">
        ...
        <div class="space-y-2">
          <div v-for="s in result.steps" :key="s.stepId" class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
        ...
        <template v-if="result.recoverySteps?.length">
          <p class="text-xs font-semibold text-gray-700 dark:text-gray-200 pt-2 border-t border-gray-200 dark:border-gray-700">Recovery steps</p>
          <div class="space-y-2">
            <div v-for="s in result.recoverySteps" :key="s.stepId" class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
              <p class="font-medium text-gray-900 dark:text-white text-sm">{{ s.name }} <span class="text-xs px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{{ s.type }}</span></p>
              <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ s.summary }}</p>
            </div>
          </div>
