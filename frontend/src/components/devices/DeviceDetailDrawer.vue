...
// Collapses the Windows OS Updates section's pending-KB list (Overview tab)
// by default, same pattern as the Compliance tab's Vulnerabilities section —
// both lists are now uncapped (backend no longer truncates to 10/15), so
// collapsing keeps a device with many pending updates from dominating the tab.
const osUpdatesPendingExpanded = ref(false);
...
// "Only risky apps" checkbox — an app counts as risky if it has at least one
// confirmed CVE (vuln.cveList.length > 0) or a non-clean integrity verdict
// (the same two signals already rendered per-row below), so the filter
...
const filteredInstalledApps = computed(() => {
  ...
  if (appsRiskyOnly.value) all = all.filter(isRiskyApp);
        ...
        <div class="flex-1 overflow-y-auto px-5 py-4">
          ...
          <template v-if="tab === 'overview'">
            ...
            <div v-if="device.osUpdateStatus" class="mb-6">
              ...
              <div v-else-if="(device.osUpdateStatus as any).pendingCount > 0" class="space-y-1.5">
                <button type="button" class="flex items-center gap-1 text-xs font-medium hover:opacity-80" :style="{ color: WARNING }" @click="osUpdatesPendingExpanded = !osUpdatesPendingExpanded">
                  <component :is="osUpdatesPendingExpanded ? ICONS.AltArrowUp : ICONS.AltArrowDown" :size="12" weight="Linear" class="shrink-0" />
                ...
                <template v-if="osUpdatesPendingExpanded">
              ...
              <div v-else-if="(device.osLifecycleStatus as any).onLatestVersion === false" class="space-y-1.5">
                ...
                <template v-if="(device.vulnStatus as any)?.pendingCount > 0">
                  ...
                  <template v-if="osUpdatesCvesExpanded">
                    ...
                    <p class="text-[10px] text-gray-400">
                      {{ (device.vulnStatus as any).pendingCount }} known CVE{{ (device.vulnStatus as any).pendingCount === 1 ? "" : "s" }} fixed in a newer version, by severity.
                    </p>
                  </template>
                </template>
          ...
          <template v-else-if="tab === 'compliance'">
            ...
            <!-- Vulnerabilities — moved here from the Overview tab per user
                 ...
                 Unified as ONE section (backend: vulnService.ts's
                 mergeOsVulnerabilities) as of this redesign — previously
                 this was two separate sections ("Vulnerabilities" sourced
                 from the EUVD catalog, and "Vulnerability Service" sourced
                 from the Worker/MISP/VulnCheck/etc.) that could flatly
                 contradict each other for the same device, and the second
                 one also mixed in app-level CVEs unrelated to the OS. Only
                 OS-level CVEs ever show here now; app-level CVEs moved to
                 the Apps tab below and the fleet-wide Apps view's Risk
                 column. -->
            <div v-if="device.osVulnerabilities && (device.osVulnerabilities as any).visible" class="mb-6">
              <p class="text-[10px] font-semibold uppercase tracking-wider mb-2 text-gray-400">Vulnerabilities</p>
              <p v-if="(device.osVulnerabilities as any).state === 'unconfirmed'" class="text-xs text-gray-400">
                No confirmed vulnerability comparison available yet for this OS version — neither the Vulnerability Catalog nor the Vulnerability Service has a parseable fixed-version match for it.
              </p>
              <p v-else-if="(device.osVulnerabilities as any).state === 'not_checked'" class="text-xs text-gray-400">
                {{ (device.osVulnerabilities as any).notCheckedDetail }}
              </p>
              <div v-else-if="(device.osVulnerabilities as any).state === 'cves'" class="space-y-1.5">
                <button type="button" class="flex items-start gap-1 text-left text-xs font-medium hover:opacity-80" :style="{ color: (device.osVulnerabilities as any).cves.some((c: any) => c.is_kev) ? DANGER : WARNING }" @click="complianceVulnCvesExpanded = !complianceVulnCvesExpanded">
                  <component :is="complianceVulnCvesExpanded ? ICONS.AltArrowUp : ICONS.AltArrowDown" :size="12" weight="Linear" class="shrink-0" />
                  {{ (device.osVulnerabilities as any).pendingCount }} known CVE{{ (device.osVulnerabilities as any).pendingCount === 1 ? "" : "s" }} against this device's OS version
                  {{ (device.osVulnerabilities as any).cves.some((c: any) => c.is_kev) ? " — includes a known-exploited (CISA KEV) CVE" : "" }}
                </button>
                <template v-if="complianceVulnCvesExpanded">
                  <div class="space-y-1.5 max-h-64 overflow-y-auto">
                    <div v-for="c in (device.osVulnerabilities as any).cves" :key="c.id" class="flex items-center justify-between gap-2 px-3 py-1.5 rounded-lg text-xs bg-gray-50 dark:bg-gray-900/50">
                      <span class="text-gray-900 dark:text-white">
              ...
              <p class="text-[10px] mt-2 text-gray-400">
                Deduplicated across the Vulnerability Catalog and your org's Vulnerability Service integration (when configured) — OS only. Per-app CVEs are on the Apps tab.
              </p>
            </div>
          ...
          <template v-else-if="tab === 'apps'">
            ...
            <template v-else>
              ...
              <label class="flex items-center gap-1.5 mb-2.5 text-xs text-gray-500 dark:text-gray-400 cursor-pointer select-none w-fit">
                <input v-model="appsRiskyOnly" type="checkbox" class="rounded border-gray-300 dark:border-gray-600" :style="{ accentColor: PRIMARY_BLUE } as any" />
                Only risky apps (CVEs or flagged integrity)
              </label>
              <p v-if="filteredInstalledApps.length === 0" class="text-xs text-gray-400 px-3 py-3 rounded-lg bg-gray-50 dark:bg-gray-900/50">
                {{ appsRiskyOnly ? "No risky apps" + (appsSearchQuery ? ` match "${appsSearchQuery}"` : " on this device") + "." : `No apps match "${appsSearchQuery}".` }}
              </p>
              <div v-else class="space-y-2.5">
              <!-- keyed on identifier alone: computeDeviceAppsDetail (vulnService.ts)
                   now merges a self-reported + Applivery-UEM sighting of the
