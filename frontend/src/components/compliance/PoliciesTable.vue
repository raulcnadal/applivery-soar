...
<template>
  <EmptyState
    ...
    "
  />
  <!-- One policy per row (not a 3-per-row card grid) — per user request:
       a policy card already surfaces a lot (condition summary, linked
       workflow, last-evaluated, live violator count, 3+ toggle chips), and
       squeezed into a third of the row's width that read as overwhelming
       rather than scannable. Full-width rows give every field room to sit
       on its own line without truncation/line-clamp tricks. -->
    ...
    <div v-for="p in policies" :key="p.id" class="rounded-xl p-4 shadow-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      <div class="flex items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p v-if="p.description" class="text-xs mt-1 text-gray-400">{{ p.description }}</p>
          ...
          <div class="flex items-center gap-x-4 gap-y-1 mt-2 flex-wrap">
            ...
            <p class="text-xs inline-flex items-center gap-1 text-gray-400" :title="p.lastEvaluatedAt ? new Date(p.lastEvaluatedAt).toLocaleString() : undefined">
