...
<template>
  <div class="space-y-6">
    <div class="space-y-3">
      <p class="text-sm font-medium text-gray-700 dark:text-gray-200">Lifecycle catalog</p>
      ...
      <div class="flex flex-wrap gap-3">
        <div v-for="p in lifecyclePlatformCounts" :key="p.platform" class="border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 px-4 py-2">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ p.platform }}</p>
          <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ p.count }}</p>
        </div>
      </div>
      <p class="text-xs text-gray-500 dark:text-gray-400">Last fetched: {{ store.osLifecycleCatalog?.lastFetchedAt ? new Date(store.osLifecycleCatalog.lastFetchedAt).toLocaleString() : "Never" }}</p>
    ...
    <div class="space-y-3 pt-4 border-t border-gray-100 dark:border-gray-800">
      <p class="text-sm font-medium text-gray-700 dark:text-gray-200">Apple GDMF (Software Lookup Service)</p>
      <p class="text-xs text-gray-400 max-w-2xl">
      ...
      <div class="flex flex-wrap gap-3">
        <div v-for="p in gdmfPlatformCounts" :key="p.platform" class="border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 px-4 py-2">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ p.platform }}</p>
          <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ p.count }}</p>
        </div>
        <div class="border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 px-4 py-2">
          <p class="text-xs text-gray-500 dark:text-gray-400">Active Rapid Security Responses</p>
          <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ gdmfRsrCount }}</p>
        </div>
      </div>
      <p class="text-xs text-gray-500 dark:text-gray-400">Last fetched: {{ store.gdmfCatalog?.lastFetchedAt ? new Date(store.gdmfCatalog.lastFetchedAt).toLocaleString() : "Never" }}</p>
