...
<template>
  ...
  <div
    ...
    style="background-color: #f3f7fe; background-image: url('https://dashboard.applivery.io/images/loading-bg.svg'); background-size: cover; background-position: center; background-repeat: no-repeat"
  >
    <div class="p-10 rounded-2xl border max-w-md w-full min-h-[450px] flex flex-col justify-center shadow-2xl relative overflow-hidden" style="background-color: #f3f7fe; border-color: #e9eaec">
      <img src="/applivery-bp-login.svg" class="h-8 mx-auto mb-8" alt="Applivery" style="filter: invert(1)" />

      <form v-if="step === 'credentials'" @submit.prevent="submitCredentials">
        <h1 class="text-xl font-normal mb-6 text-center text-gray-900">Welcome Back</h1>
        ...
        <div class="mb-4">
          <label class="block text-xs font-medium mb-1 text-gray-500">Email</label>
          <input
            v-model="email"
            type="email"
            required
            ...
            class="w-full rounded-lg px-3 py-3 outline-none text-sm border border-gray-200 bg-white focus:border-blue-500 transition-colors focus:ring-2 focus:ring-brand-500"
          />
        </div>
        <div class="mb-6">
          <label class="block text-xs font-medium mb-1 text-gray-500">Password</label>
          <input
            v-model="password"
            type="password"
            required
      ...
      <div v-else-if="step === 'mfa'">
        <button type="button" class="absolute top-6 left-6 text-gray-500 hover:opacity-70 transition-opacity" @click="backToCredentials">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
        </button>
        <h1 class="text-xl font-bold mb-1 text-center text-gray-900">Two-Factor authentication</h1>
        <p class="text-sm mb-6 text-center text-gray-500">Enter the six-digit code generated by your Authenticator App.</p>
          ...
          <div class="mb-6">
            <div class="flex gap-2" @paste="handleDigitPaste">
              <input
                ...
                :value="digit"
                :autofocus="i === 0"
                class="flex-1 min-w-0 aspect-square rounded-xl text-center text-xl font-semibold border-2 border-gray-200 bg-white outline-none transition-all focus:border-blue-500 focus:ring-2 focus:ring-brand-500"
      ...
      <div v-else-if="step === 'workspace'">
        <h1 class="text-xl font-bold mb-1 text-center text-gray-900">Select Workspace</h1>
        <p class="text-sm mb-6 text-center text-gray-500">Your account has access to multiple workspaces. Choose one to continue.</p>
        ...
        <div class="space-y-2 max-h-72 overflow-y-auto">
          <button
            ...
            type="button"
            :disabled="isLoading"
            class="w-full text-left px-4 py-3 rounded-xl border-2 border-gray-200 hover:border-brand-500 transition-colors text-sm font-medium text-gray-900"
