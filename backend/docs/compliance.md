...
| Inbound Webhook Fired | pick an enabled [Inbound Webhook](settings.md#inbound-webhooks) — exists = currently active (fired, not yet Resolved by the same external system), missing = resolved or never fired. Tracks a real Fired/Resolved lifecycle, not just a one-way "did it ever fire" flag — a device recovers automatically once the external tool calls the webhook's Resolve URL, the same way it would for any other condition clearing. Optional "auto-expire after N minutes" is a staleness safety net for a tool that can't call back on resolution, not the primary signal |
