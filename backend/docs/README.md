...
This build is a Vue 3 single-page frontend and a Node.js/Express + TypeScript backend, backed by Postgres via Prisma ORM. Every account is an existing Applivery Collaborator — there's no separate user database, and every device/policy action ultimately calls Applivery's own management API on your behalf.
...
See [ARCHITECTURE.md](ARCHITECTURE.md) for how the codebase itself is put together.
