...
**Status:** Phases 0–3 implemented (2026-08-15) — Windows Agent has both a config-driven registry watcher (Phases 0–2: `EventWatchDefinition` admin CRUD, `GET /api/device-data/event-watches`, `POST /api/device-data/event-notify`, `eventwatch_windows.go`) and a config-driven ETW watcher (Phase 3: `watchType: "etwProvider"`, `etw_windows.go`, using `github.com/0xrawsec/golang-etw`). Phase 1's hardcoded-watcher step was deliberately skipped in favor of going straight to Phase 2, per direction — no throwaway code. Phase 4 (rollout controls/metrics) and Phase 5 (macOS) remain future work.
