...
**Status:** Phases 0–4 implemented (2026-08-15) — Windows Agent has a config-driven registry watcher (Phases 0–2: `EventWatchDefinition` admin CRUD, `GET /api/device-data/event-watches`, `POST /api/device-data/event-notify`, `eventwatch_windows.go`), a config-driven ETW watcher (Phase 3: `watchType: "etwProvider"`, `etw_windows.go`, using `github.com/0xrawsec/golang-etw`), and Phase 4's rollout controls: a per-workspace kill switch (`WorkspaceState.eventDrivenDetectionEnabled`), a remote `IntervalSec` override delivered through the same poll response and hot-applied via `ticker.Reset` (no agent restart — `telemetry_windows.go`'s `maybeResetTicker`), and notify metrics (webhook volume, debounce-collapse ratio, event-to-reaction latency — `EventNotifyMetric`, Settings > Event-Driven Detection's "Metrics" panel). Phase 1's hardcoded-watcher step was deliberately skipped in favor of going straight to Phase 2, per direction — no throwaway code. Phase 5 (macOS) remains future work.
