...
**Connecting a Git repo** ("Custom repo…" inside the Import from Git repo modal): pick a **Vendor** — GitHub, GitLab, or Custom/self-hosted (any server whose API mirrors GitHub's Contents API shape, which covers GitHub Enterprise Server and Gitea) — then paste the repo's **URL** (e.g. `https://github.com/owner/repo` or `https://gitlab.mycompany.com/group/project`) to auto-fill the Owner/Repository fields below it; both stay editable if the parse guesses wrong. A non-default host (anything other than github.com/gitlab.com) also auto-fills the **API base URL** field, which is required for a Custom connection since there's no public default to fall back to. **Branch** and an optional **Path** scope which subtree gets browsed. An optional **Access token** (personal/project token, sent as `Authorization: Bearer` for GitHub/Custom or `PRIVATE-TOKEN` for GitLab) is only needed for a private repo — it's encrypted at rest and never sent back to the browser; the connected-repos list just shows a lock icon when one is set.
