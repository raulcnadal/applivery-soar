...
**`ssl_client_certificate` path pitfall on NPM (a second live incident, still not fully root-caused).** After the dedicated-subdomain fix above and the `RegisterURL` split, a pilot device still hit `tls: unrecognized name` against the agents subdomain — reproduced independently with `curl -vI` from an unrelated machine, so it wasn't Windows- or network-specific. The proxy host's Advanced config referenced `ssl_client_certificate /app/soar-ca.pem;`. `/app` is NPM's own application code directory inside its container, not a volume — the official `docker-compose.yml` only persists `./data:/data` and `./letsencrypt:/etc/letsencrypt` (confirmed against NPM's own quick-setup docs). A file placed at `/app/...` can be unreadable to nginx, or simply gone after the container is recreated, even though the proxy host shows "enabled"/green — that flag only reflects Let's Encrypt issuance, not whether every Advanced-tab directive resolved successfully. Removing the `ssl_verify_client`/`ssl_client_certificate` lines restored a normal handshake, which is consistent with (but doesn't 100% prove) a bad file path being the cause — NPM also has its own long-standing, still-open, label-`bug` GitHub issue ([NginxProxyManager/nginx-proxy-manager#3752](https://github.com/NginxProxyManager/nginx-proxy-manager/issues/3752)) describing the identical `unrecognized name`/SSL-alert-112 failure on a vanilla proxy host with no client-cert config at all, so an unrelated NPM SNI/cert-dispatch flake can't be ruled out either. Worth noting: NPM does not yet ship first-class client-certificate support in any released version — [issue #768](https://github.com/NginxProxyManager/nginx-proxy-manager/issues/768) (opened 2020) requesting exactly that is still open, with two draft PRs (#2956, #5563) unmerged — so hand-editing `ssl_verify_client`/`ssl_client_certificate` via the Advanced tab remains the only available method on NPM today, fragility and all. The proxy-config reference (this doc and `MtlsPanel.vue`'s generated snippet) now points at `/data/soar-ca.pem` instead of a bare `/path/to/...` placeholder, with an inline note to verify the file's presence via `docker exec <npm-container> ls -la /data/soar-ca.pem` before assuming the directive itself is at fault.
