 ...
 * /api/audit-logs/export. Deliberately no requirePermission gate beyond the
 * dashboard-token check, matching the original (the `auditLog` RBAC area is
 * declared but not actually enforced anywhere — migration-plan.md §9).
 */
...
auditLogsRouter.get("/api/audit-logs", verifyDashboardToken, asyncHandler(async (req, res) => {
  const limit = req.query.limit ? Number(req.query.limit) : 100;
  const offset = req.query.offset ? Number(req.query.offset) : 0;
  res.json(await listAuditLogs(workspaceOf(req), filtersOf(req), limit, offset));
}));

auditLogsRouter.get("/api/audit-logs/actors", verifyDashboardToken, asyncHandler(async (req, res) => {
  res.json({ items: await listAuditLogActors(workspaceOf(req)) });
}));

auditLogsRouter.get("/api/audit-logs/export", verifyDashboardToken, asyncHandler(async (req, res) => {
