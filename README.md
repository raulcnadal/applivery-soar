...
| `AGENT_BUILD_INGEST_SECRET` | Optional, but required for the zero-config native agent download to work | A single operator-held secret (not per-workspace) that gates `POST /api/internal/agent-builds/:platform` — the endpoint the two native agent repos' own CI POSTs freshly-built binaries to. Set the same value as the `SOAR_AGENT_BUILD_SECRET` repository secret in both [`applivery-soar-agent-windows`](https://github.com/raulcnadal/applivery-soar-agent-windows) and [`applivery-soar-agent-macos`](https://github.com/raulcnadal/applivery-soar-agent-macos). Leave unset and the ingest endpoint stays fail-closed (503) — Settings → Applivery SOAR Agent will show no build yet, and the older GitHub-token download path still works as a fallback. See `ARCHITECTURE.md` §9.2. |
